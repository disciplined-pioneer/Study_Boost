...
async def send_user_information(message: types.Message, state: FSMContext, data, user_id, payment_photo):
    ...
    await message.reply('Ваши данные были отправлены администратору. Ожидайте результата проверки 😊')
    ...
    if result:
        ...
        if user_payment:
            ...
            if expiration_date > now_date:
                await message.reply(f'Добро пожаловать! Доступ к платформе открыт 😊', reply_markup=platform_menu)
            else:
                await state.clear()
                await message.reply(f'Ваша подписка не оплачена! Пожалуйста, отправьте фото с вашей оплатой: ')
                await state.update_data(user_info=user_info, user_id=user_id)
                await state.set_state(PaymentStates.payment_photo)
        else:
            await message.reply('Не найдены данные о платеже. Пожалуйста, свяжитесь с поддержкой.')
    else:
