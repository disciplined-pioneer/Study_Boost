...
async def registration_handler(message: Message, state: FSMContext):
    ...
    if can_use == 0:
        await message.reply(
            "Для отмены действия нажмите кнопку *«Отменить ❌»*.\n\n⚠️ Важно: если ранее вы уже использовали кнопку *«Отменить ❌»* или команду */cancellation*, ваш реферальный ID был сброшен.\n\nЧтобы воспользоваться бонусами реферальной ссылки, перейдите по ней ещё раз.",
            parse_mode="Markdown"
        )
        await start_registration(message, state)
